...
class SimpleTokenizer(object):
    def __init__(
        ...
        # BPE merge rules (kept but not necessarily used in encode)
        ...
        self.encoder = {tok: idx for idx, tok in enumerate(vocab)}
        self.vocab_size = len(vocab)
        self.decoder = {idx: tok for tok, idx in self.encoder.items()}
        ...
        # Split regex (preserve special tokens)
        special = "|".join(map(re.escape, special_tokens))
        self.pat = re.compile(rf"(?:{special})|\S+", re.IGNORECASE)
    ...
    def encode(self, text: str) -> List[int]:
        ...
        words = text.split(' ')
        return [self.encoder.get(w, self.eot_token_id) for w in words]

    def decode(self, tokens: List[int]) -> str:
        text = ''.join(self.decoder.get(t, '') for t in tokens)
        text = bytearray(self.byte_decoder.get(c, c) for c in text) \
    ...
    def __call__(self, texts: Union[str, List[str]], context_length: Optional[int] = None) -> torch.LongTensor:
        ...
        # Global check max_id
        max_id = max(max(tokens) for tokens in all_tokens)
        if max_id >= self.vocab_size:
            raise IndexError(f"Found token id {max_id} >= vocab_size {self.vocab_size}!")
...
def get_tokenizer(
        ...
        **kwargs,  # can receive other SimpleTokenizer parameters
):
    """
    Gets the appropriate tokenizer based on the model identifier schema or name.
    """
    ctx = context_length or DEFAULT_CONTEXT_LENGTH
    ...
    if special_tokens_path is not None:
        try:
            with open(special_tokens_path, "r", encoding="utf-8") as f:
                additional_special_tokens = [line.strip() for line in f if line.strip()]
            logging.info(f"Loaded {len(additional_special_tokens)} special tokens from {special_tokens_path}")
        except Exception as e:
            logging.warning(f"Failed to load special tokens from {special_tokens_path}: {e}")
